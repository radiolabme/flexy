...
func resetMetering() {
	...
	meterMu.Lock()
	defer meterMu.Unlock()
	meterVal = map[string]float64{}
	meterMeta = map[string]flexclient.Object{}
	flexToHamlib = map[string]string{}
	hamlibToFlex = map[string]string{}
}
...
func meterPacket(pkt flexclient.VitaPacket) {
	...
	for {
		...
		log.Trace().Int("id", int(id)).Int("rawval", int(rawVal)).Float64("val", val).Str("hamlib", hamlibName).Interface("meta", meta).Msg("meter data")
		meterVal[name] = val
	}
}
